...
from src.data import default_multilingual_specs, load_hf_dataset


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default="xlm-roberta-base")
    parser.add_argument("--max_samples", type=int, default=200, help="per-language sample cap for speed")
    ...
    specs = default_multilingual_specs()
    df_all = []
    for spec in specs:
        df_all.append(load_hf_dataset(spec, max_samples=args.max_samples))
    df = pd.concat(df_all, ignore_index=True)

    scorer = PLLScorer(model_name=args.model)

    scores = []
    for s in tqdm(df["sentence"].tolist(), desc="Scoring sentences (PLL)"):
